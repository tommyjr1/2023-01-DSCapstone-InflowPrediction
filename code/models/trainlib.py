...
def lagndelay(num_lags, delay, df, y):
    ...
    mask = (df.columns.str.contains(y) | df.columns.str.contains('lag') | df.columns.str.contains('window'))
...
def split(ratio, df):
    df_train = df.loc[:int(df.shape[0]*ratio),:]
    df_test = df.loc[int(df.shape[0]*ratio):,:]
